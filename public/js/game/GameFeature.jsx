...
function bankSoal(){return bankGabungan(BANK_SOAL,"kuis");}
function bankAyat(){return bankGabungan(BANK_AYAT,"susun");}
function bankTokoh(){return bankGabungan(BANK_TOKOH,"tebak");}
function bankKartu(){return bankGabungan(BANK_KARTU,"memory");}
...
function siapkanSoal(n){return pilihSegar(bankSoal(),n,"soal",s=>s.q).map(s=>{const b=s.opsi[s.benar];const o=shuffle(s.opsi);return{q:s.q,opsi:o,benar:o.indexOf(b)};});}
function siapkanAyat(n){return pilihSegar(bankAyat(),n,"ayat",a=>a.ref).map(a=>{const kata=a.teks.split(" ");return{ref:a.ref,kata,acak:shuffle([...kata])};});}
function siapkanTokoh(n){return pilihSegar(bankTokoh(),n,"tokoh",t=>t.jawaban).map(t=>{let op=shuffle([t.jawaban,...t.salah]).slice(0,4);if(!op.includes(t.jawaban))op[0]=t.jawaban;return{...t,opsi:shuffle(op)};});}
function siapkanKartu(n=8){const p=pilihSegar(bankKartu(),n,"kartu",x=>x.a);return shuffle([...p.map((x,i)=>({id:i*2,pair:i,isi:x.a})),...p.map((x,i)=>({id:i*2+1,pair:i,isi:x.b}))]);}
...
/* ── PERAYAAN HASIL — animasi + kata sesuai hasil (menang/seri/kalah),
   tampil di layar hasil akhir. Ukuran dipatok pas untuk kartu game
   (maxWidth 460px) — GIF 32x35px asli di-scale besar oleh SCALE=12 di
   generator-nya, jadi cukup dibatasi lebar tampil di sini. */
const PERAYAAN_HASIL={
  menang:{src:"/js/game/yesus_bersorak.gif",alt:"Bersorak untuk kemenanganmu",kutipan:`"Bersorak-sorailah bagi TUHAN, hai bumi!" — Mazmur 100:1`,rasio:"32/35",lebar:120},
  seri:{src:"/js/game/yesus_seri.gif",alt:"Menemanimu dengan sukacita",kutipan:`"Sebab di mana dua atau tiga orang berkumpul dalam nama-Ku, di situ Aku ada di tengah-tengah mereka." — Matius 18:20`,rasio:"32/35",lebar:120},
  kalah:{src:"/js/game/yesus_kalah.gif",alt:"Menguatkanmu untuk mencoba lagi",kutipan:`"Sebab tujuh kali orang benar jatuh, namun ia bangun kembali." — Amsal 24:16`,rasio:"32/35",lebar:120},
...
function KuisSolo({onExit}){
  ...
  if(selesai)return<Hasil judul="Selesai! 🎉"skor={skor}accent={P.gold}onExit={onExit}hasil="selesai"baris={[{label:"Jawaban benar",val:`${benarTotal}/10`},{label:"Streak terbaik",val:`${best} 🔥`},{label:"Akurasi",val:`${Math.round(benarTotal/10*100)}%`}]}/>;
...
function HasilN({players,scores,myId,accent,onExit}){
  ...
  const puncak=skorSaya===topScore;
  // Puncak bersama lebih dari 1 orang = seri (bukan menang outright);
  // puncak sendirian = menang; selain itu = kalah.
  const jumlahPuncak=players.filter(p=>(scores[p.id]??0)===topScore).length;
  const hasil=!puncak?"kalah":jumlahPuncak>1?"seri":"menang";
  const menang=hasil==="menang";
  const medals=["🥇","🥈","🥉","4️⃣"];
...
function SusunSolo({onExit}){
  ...
  const [disusun,setDisusun]=useState([]);const [bank,setBank]=useState([]);const [waktu,setWaktu]=useState(90);const [selesai,setSelesai]=useState(false);
  ...
  const lanjut=useCallback((berhasil)=>{clearInterval(timerRef.current);if(berhasil){const p=100+Math.round((waktu/90)*150)+streak*25;setSkor(s=>s+p);setBenarTotal(b=>b+1);setStreak(s=>s+1);setFlash({ok:true,pesan:`+${p} poin!`});}else{setStreak(0);setFlash({ok:false,pesan:"Waktu habis ⏱"});}setTimeout(()=>{setFlash(null);if(idx+1>=ayat.length)setSelesai(true);else{setIdx(i=>i+1);setWaktu(90);}},1400);},[waktu,streak,idx,ayat.length]);
  ...
  if(selesai)return<Hasil judul="Selesai! 📖"skor={skor}accent={P.p2}onExit={onExit}hasil="selesai"baris={[{label:"Ayat tersusun",val:`${benarTotal}/5`},{label:"Streak terbaik",val:`${Math.max(benarTotal,0)} 🔥`},{label:"Total poin",val:skor.toLocaleString()}]}/>;
...
function SusunTatap({lawan,onExit}){
  ...
  const [waktu,setWaktu]=useState(90);const [winner,setWinner]=useState(null);const [selesai,setSelesai]=useState(false);const timerRef=useRef();
  const a=ayat[idx];
  useEffect(()=>{const bank=a.acak.map((k,i)=>({kata:k,origIdx:i}));setState({p1:{disusun:[],bank:[...bank]},p2:{disusun:[],bank:[...bank]}});},[idx]);
  const lanjut=useCallback(w=>{if(idx+1>=ayat.length)setSelesai(true);else{setIdx(i=>i+1);setWinner(null);setWaktu(90);};if(w)setSkor(s=>({...s,[w]:s[w]+100+Math.round((waktu/90)*80)}));},[idx,ayat.length,waktu]);
...
function TebakSolo({onExit}){
  const [tokoh]=useState(()=>siapkanTokoh(10));const [idx,setIdx]=useState(0);const [skor,setSkor]=useState(0);const [benarTotal,setBenarTotal]=useState(0);
  ...
  if(selesai)return<Hasil judul="Selesai! 🔍"skor={skor}accent={P.purple}onExit={onExit}hasil="selesai"baris={[{label:"Tokoh tertebak",val:`${benarTotal}/${tokoh.length}`},{label:"Akurasi",val:`${Math.round(benarTotal/tokoh.length*100)}%`},{label:"Total poin",val:skor.toLocaleString()}]}/>;
...
function TebakTatap({lawan,onExit}){
  const [tokoh]=useState(()=>siapkanTokoh(10));const [idx,setIdx]=useState(0);const [skor,setSkor]=useState({p1:0,p2:0});
...
function TebakOnline({players,sessionCode,hindariKeys,hostId,onExit}){
  ...
  useEffect(()=>{
    resolvedRef.current=false;waktuRef.current=25;setWaktu(25);
  },[idx]);
...
function MemorySolo({level,onExit}){
  ...
  if(selesai)return<Hasil judul="Semua Cocok! 🃏"skor={skor}accent={P.orange}onExit={onExit}hasil="selesai"baris={[{label:"Waktu",val:`${mnt}:${dtk}`},{label:"Langkah",val:langkah},{label:"Pasangan",val:`${matched.length/2}/${cfg.pasang}`}]}/>;
